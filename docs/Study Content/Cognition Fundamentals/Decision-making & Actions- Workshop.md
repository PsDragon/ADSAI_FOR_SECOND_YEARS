...
Today we're going to determe which cognitive systems are involved in the bin-picking tasks and how they work. Then we're going translate to that to a robot doing the task.
...
After first re-iterating on the learning objectives from the previous cognition datalab:
