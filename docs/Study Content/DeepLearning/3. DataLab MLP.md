...
where $x$ is the feature value we would like to normalize, $x_{min}$ is the smallest value of the feature and $x_{max}$ is the largest value of the feature.
...
# Looping the folder with glob
for fpath in glob.glob('dataset/train/*.jpg'):
    # Opening images with OpenCV
    # You can use anything you would like such as PIL
    im = cv2.imread(fpath, 0)
    
    # If the image label contains frying
    # label = 1
