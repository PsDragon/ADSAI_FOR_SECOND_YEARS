...
To create our vertical slice, we first need to define what the user can actually do using this core functionality; which is typically listed as user storties. So to define what would make up our vertical slice we first need to define **user stories**. Then we can use our user stories and interaction design knowledge to build a wireframe prototype using proto.io! A vertical slice is essentially the core of your application prototyped and only that. For you, that means that you design the interaction around how you are going to use the deep learning algorithm; the core functionality which is going to do the computer vision magic.
...
So, we already made the algorithm which contains our core functionality and we've gather our dataset which represents our Proof-of-Concept (PoC). Ofcourse, doing the programming; turning our Juypter notebook into .py scripts and integrating them in an API, to actually create the real application is something outside the scope of this project (that'll come next block). However, the UI and UX is something we can design, prototype test already for our envisoned application. That way we can detect early design flaws and change them before we would actually start building; if we would actually create an API from our PoC. So, to start designing the UX; and UI, we need to know what the user needs and wants to do, so let's **learn to define user-stories by watching the video below:**
...
Interaction is all about change: by exchanging information in the form of data or energy we're creating some kind of new state in the world. I can for example gain new information in the form of data by vision, sound or vibrations; this new information makes the user aware of something; changing the internal state of the user. Or I can exchange information in the form of energy by kicking a football and which after my interaction will interact with the environment and change it. What it essentially comes down to when you are designing for interaction is that you create some kind of change which the user can anticipate and understand without thinking, in other words: design should be intuitive.
...
So for the beer bottle scanner app, this would just be the camera and what the user can do there; which user stories are connected to using that feature? I 'll probably have multiple screen if I can do multiple things with my core functionality or when my core functionality is realized in multiple steps; say a search query.

Make sure to finish your vertical slice today! 

## Next up!
Coming Datalab we are going to test your vertical slice by doing our first user-test: Wizzard of Oz Prototyping. We are also going to learn more on what sets HCAI apart from regular human-computer interaction!
