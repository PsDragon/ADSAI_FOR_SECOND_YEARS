...
- Please complete the following course on DataCamp to learn more about Docker: [Introduction to Docker](https://app.datacamp.com/learn/courses/introduction-to-docker) 📚
        ...
        ⚠️ Note: you only need to do the first 3 chapters.
...
# Set the Python application as an executable
ENTRYPOINT ["poetry", "run", "your-cli-app"]
CMD ["--help"]
```

- `FROM python:3.9`: This line indicates that the base of this Docker image will be the official Python 3.9 image.

- `WORKDIR /app`: Sets the working directory in the Docker container to `/app`.

- `LABEL maintainer="your_email@example.com"` and other `LABEL` lines: These lines set metadata for the Docker image. This includes information like who maintains the image, its version, and a description.

- `COPY . /app`: This command copies everything from the current directory on your local machine (where your Dockerfile is) into the `/app` directory inside the Docker container.

- `RUN pip install poetry`: This line installs the poetry tool inside the Docker container.

- `RUN poetry config virtualenvs.create false && poetry install`: This configures poetry not to create a virtual environment, then installs the Python dependencies specified in your `pyproject.toml` file.

- `ENTRYPOINT ["poetry", "run", "your-cli-app"]`: This specifies that the entry point of the container is our CLI application. Replace `your-cli-app` with the command you use to run your CLI app. 

- `CMD ["--help"]`: The `CMD` line is providing a default argument to the `ENTRYPOINT` command, in this case `--help`. This means that if you run the Docker container without specifying any commands, it will default to showing the help menu of your CLI app.
...
Docker containers are isolated from the host system by default, which means they don't have access to your local file system unless you explicitly allow it.
...
```bash
your-cli-app predict /path/in/container/image_file.jpg
```

This command will run the `predict` command of your CLI app on `image_file.jpg` inside the Docker container.
