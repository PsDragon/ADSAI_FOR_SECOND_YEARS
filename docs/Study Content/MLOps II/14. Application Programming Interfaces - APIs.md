...
INCLUDING INFO ON AUTO API DOCS

To access the auto-generated API documentation, navigate to `http://localhost:8000/docs` in your browser.
...
This API is currently hosted on azure - your can try it out [here]()!

You can also interact with the API using Python, specifically with the `requests` library which allows you to send HTTP requests in Python. Here's how you might do it to upload an image file from your local machine:
...
## Deploying your API in a Docker container 🐳 (work in progress 🚧)
