...
This API is currently hosted on Azure. You can try it out by clicking on the following [link]()!
...
## 6. Deploying your API in a Docker container 🐳 (work in progress 🚧)
...
LABEL version="1.0"
LABEL description="FastAPI app to predict MNIST digits"
...
# This line is not required, but it's good practice to include it
EXPOSE 80

# Define the command to start the container. Note that we're using uvicorn to run our app, specify the location of the app, 
# and we bind it to 0.0.0.0 so it can accept requests from outside the container.
ENTRYPOINT ["uvicorn", "src.number_predictor.app:app", "--host", "0.0.0.0", "--port", "80"]
```
In this Dockerfile:
...
2. `src.number_predictor.app:app`: This is the location of your FastAPI application. The format is `module:variable`. In this case, the module is `src.number_predictor.app` (we want to run the ` app.py` file) and the variable the FastAPI class is assigned to is `app`.
