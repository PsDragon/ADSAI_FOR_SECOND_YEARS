...
| 5 | Machine Learning Analysis | Content, Twitter, and Ratings Data | Banijay & Twitter API
...
You need to find the ```mean``` ratings per fragment using ```kdh1000``` as an indicator for how well a show is performing, and focus on the ```totaal``` ratings type.
...
> The tweets are posted by @npo1; the broadcaster of the show Op1.
