...
|----------|-------------|-------------|-------------|-------------|
| 1 | Pre-process and clean data | Content and Ratings Data | Banijay | [clicky](#use-case-1-linking-content-data-to-ratings-data)
| 2 | Merge data | Content and Ratings Data | Banijay | [clicky](#use-case-2-linking-content-data-to-ratings-data)
| 3 | Content and Ratings exploration | Content and Ratings Data | Banijay | [clicky](#use-case-3-content-ratings-analysis)
| 4 | Integrating Social Media Buzz | Twitter_data | Twitter API | [clicky](#use-case-4-social-media-analysis)
| 5 | Machine Learning Analysis | Content, Twitter, and Ratings Data | Banijay & Twitter API | [clicky](#use-case-5-machine-learning-analysis)
...
- [ ] read in the ```banijay_op1data_twitter_raw.json``` file from [here](https://edubuas.sharepoint.com/:u:/t/2022-23BFAI1.P2-01ADSAI/EUrNefNkNe1DkwIQQlZOmOIBwqyofoaSa6TVHkdmx60ZfQ?e=Ia8iYk) and store it in a dataframe. Hint: after reading in the data using ```pd.read_json```, you will need to normalize it using ```pd.json_normalize()```. Read the documentation for more information.
- [ ] create a lookup table that allows you link the content and the social media data. In simpler terms, every tweet in the social media data should be linked to a show (a ```content_ID```).
...
In this use-case, we will use all the datasets we have encountered so far to build a machine learning model that predicts the ratings of a show based on the content of the show.
...
- [ ] Build a machine learning model to predict the ratings of a show based on the content of the show.
- [ ] Evaluate the performance of your model on the test set using the performance metric you defined earlier.
- [ ] Visualize the results of your model. For example, you can plot the predicted ratings against the actual ratings.
- [ ] Summarize your findings in the report


If you seek a challenge, you can:

- [ ] Try to improve the performance of your model by tuning the hyperparameters of your model.
- [ ] Try to improve the performance of your model by different models and compare the results.
