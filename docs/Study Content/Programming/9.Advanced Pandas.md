...
- [ ] convert the Date and Time (check the dataset to identify columns representing time) columns into datetime objects and create a ```date_time``` column that combines the two columns.
