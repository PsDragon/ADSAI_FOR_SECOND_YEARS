...
### 2.2 Sensors
Sensors are the means by which a robot perceives it's environment. The complexity and number of sensors is often directly related to the complexity of the required task. In this example the robot may need to know its location within the environment (warehouse), the distance to nearby objects and possibly what these objects are. A GPS could be used to detect the robots location, and a scanning LiDAR could be used to provide the distances to all nearby objects in the desired field of view. In order to determine what the objects are, a camera could be used in conjunction with a computer vision algorithm for object detection and classification. 
...
| Orientation | Magnetometer | Heading | A magnetometer is essentially a compass that can be used to determine a robots heading. |
...
| --- | --- | --- | --- |
| DC Motor | A DC motor is a motor that uses direct current electricity to spin a rotor. The rotor can be a permanent magnet or a coil of wire. The rotor spins in a magnetic field created by a stator. The magnetic field can be created by a permanent magnet or a coil of wire. | DC motors are commonly used in robotics to provide rotational motion. They are also used to provide linear motion in the form of a linear actuator. |
| Servo Motor | A servo motor is a DC motor that is controlled by a pulse width modulated (PWM) signal. The PWM signal controls the speed and direction of the motor. | Servo motors are commonly used in robotics to provide rotational motion. They are also used to provide linear motion in the form of a linear actuator. |
| Stepper Motor | A stepper motor is a motor that uses a series of pulses to move the rotor. The pulses are used to control the speed and direction of the motor. | Stepper motors are commonly used in robotics to provide rotational motion. They are also used to provide linear motion in the form of a linear actuator. |
| Hydraulic Cylinder | A hydraulic cylinder is a cylinder that is filled with a hydraulic fluid. The hydraulic fluid is pressurized and the pressure is used to move the piston in the cylinder. | Hydraulic cylinders are commonly used in robotics to provide linear motion. |
| Linear Actuator | A linear actuator is a motor that is used to move a piston in a cylinder. The piston is attached to a rod which can be used to move a load. | Linear actuators are commonly used in robotics to provide linear motion. |

| Motors | Motors are used to provide motion to the robot. Motors can be used to provide linear motion (e.g. wheels) or rotational motion (e.g. motors for a robotic arm). |  |
| Servo Motors | Servo motors are motors that can be controlled to move to a specific position. Servo motors are often used to control the position of a robotic arm. |  |
| Stepper Motors | Stepper motors are motors that can be controlled to move to a specific position. Stepper motors are often used to control the position of a robotic arm. |  |
| Hydraulic Cylinders | Hydraulic cylinders are used to provide linear motion. They are often used to control the position of a robotic arm. |  |
| Pneumatic Cylinders | Pneumatic cylinders are used to provide linear motion. They are often used to control the position of a robotic arm. |  |
| Linear Actuators | Linear actuators are used to provide linear motion. They are often used to control the position of a robotic arm. |  |
