...
Another example of bang-bang control is the cruise control system in a car. The cruise control system maintains the desired speed setpoint by accelerating (bang) or decelerating (bang) as needed. If the car's speed falls below the setpoint, the cruise control system will increase the throttle (bang) to bring the speed back up. If the car's speed rises above the setpoint, the cruise control system will decrease the throttle (bang) to prevent the speed from getting too high.

Bang-bang control is a simple and effective control method, but it can be somewhat crude and can result in oscillations around the setpoint. More advanced control algorithms, such as PID (proportional-integral-derivative) control, can be used to achieve better performance and smoother control.
...
The following is a text summary to refer to if you prefer to read rather than watch the videos.
...
<details> 
  <summary>Click for Answers </summary>
   ...
   <br>
   1.3b solution: A PD controller is appropriate when the system being controlled is very fast and the proportional term is sufficient to eliminate the steady state error. AAs an example an inverted pendulum system can be controlled with a PD controller if you want to balance it perfectly upright.
</details>
</div>
